<template>
  <div class="content">
    <button class="add-card" @click="addToCard()">
      Add to Cart </button>
    <div class="top-row">
...
/*
quick bindings:
  can replace => v-bind:src for :src,
  and            v-on:click for @click

   v-for
   v-once => updated once when de page is rendered
   v-if => show if the condition is true. add or remove element from de the dom
           uses: if the content is not expensive to render

   v-show => show if the condition is true. add a display none if the condition is false
             uses: if the content is expensive to render (like child component)
*/
...
function getNexValidIndex(index, length) {
...
export default {
  ...
  methods: {
    ...
    selectNextHead() {
      this.selectHeadIndex = getNexValidIndex(
    ...
    selectNextLeftArm() {
      this.selectLeftArmIndex = getNexValidIndex(
        this.selectLeftArmIndex,
        availableParts.arms.length,
      );
    },
    ...
    selectNextTorso() {
      this.selectTorsoIndex = getNexValidIndex(
        this.selectTorsoIndex,
        availableParts.torsos.length,
      );
    },
    ...
    selectNextRightArm() {
      this.selectRightArmIndex = getNexValidIndex(
        this.selectRightArmIndex,
        availableParts.arms.length,
      );
    },
    ...
    selectNextBase() {
      this.selectBaseIndex = getNexValidIndex(
        this.selectBaseIndex,
        availableParts.bases.length,
      );
    },
